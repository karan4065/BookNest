...
export default function AdmOrders() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await axios.get("https://booknest-2-k8rn.onrender.com/product/orders");
  ...
  const updateStatus = async (order) => {
    ...
    try {
      await axios.patch(`https://booknest-2-k8rn.onrender.com/product/orders/${id}`, {
        status: newStatus,
      });
  ...
  const acceptCancellation = async (order) => {
    ...
    try {
      await axios.delete(`https://booknest-2-k8rn.onrender.com/product/orders/${id}`);
