...
export default function AdmOrders() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await axios.get("https://booknest-1-7v5a.onrender.com/product/orders");
  ...
  const updateStatus = async (order) => {
    ...
    try {
      await axios.patch(`https://booknest-1-7v5a.onrender.com/product/orders/${id}`, {
        status: newStatus,
      });
  ...
  const acceptCancellation = async (order) => {
    ...
    try {
      await axios.delete(`https://booknest-1-7v5a.onrender.com/product/orders/${id}`);
